...
def energy_conservation_error(dataset_path: str, integrator_order=4) -> float:
    """
    ...
    with h5py.File(dataset_path, 'r') as f:
        ...
        return abs((E_end - E0) / E0)

def _hamiltonian(m, p, v):
    # Kinetic
    ...
    for i in range(n):
        for j in range(i+1, n):
            dist = np.linalg.norm(p[i] - p[j]) + 1e-10
            pe -= (m[i] * m[j]) / dist
    return ke + pe
...
def calculate_tes_score(mse: float, energy_drift: float, speedup: float) -> float:
    ...
    # Safe logs
    s_mse = -np.log10(mse + 1e-20)
    s_phys = -np.log10(energy_drift + 1e-20)
    s_speed = np.log10(speedup + 1e-20)
...
def butterfly_score(model_fn, initial_state, dt, steps, true_lyapunov=0.0) -> float:
    """
    ...
    # d(t)
    diff = np.linalg.norm(p_base - p_new) # Frobenius norm of pos matrix diff
    d0 = np.linalg.norm(p0 - p_pert)
    ...
    if diff < 1e-15: return 0.0 # No divergence (Over-stable)
    
    lambda_est = (1.0 / t_total) * np.log(diff / d0)
