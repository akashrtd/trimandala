...
def train():
    ...
    val_dataset = NBodyDataset("data/val.h5")
    ...
    if HAS_CODECARBON:
        ...
        tracker = EmissionsTracker(
            output_dir="metrics",
            output_file="mlp_training_emissions.csv",
            log_level="error",
        )
        tracker.start()
    ...
    for epoch in range(EPOCHS):
        ...
        print(
            f"Epoch {epoch + 1}: Train Loss {avg_train_loss:.2e} | Val Loss {avg_val_loss:.2e} | LR {optimizer.param_groups[0]['lr']:.2e}"
        )
    ...
    print(f"Best Val Loss: {best_loss:.2e}")
    ...
    print(f"Training metadata saved to models/mlp_baseline_metadata.json")
